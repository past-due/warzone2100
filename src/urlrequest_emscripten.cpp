...
public:
	...
	{
		onSuccess(HTTPResponseDetails(CURLE_OK, status, responseHeaders));
	}

	virtual void handleRequestError(unsigned short status) override
	{
		onFailure(URLRequestFailureType::TRANSFER_FAILED, HTTPResponseDetails(CURLE_HTTP_RETURNED_ERROR, status, responseHeaders));
	}
			...
			case URLRequestFailureType::TRANSFER_FAILED:
				...
				else
				{
					...
					wzAsyncExecOnMainThread([url, result, httpStatusCode]{
						debug(LOG_NET, "Fetch: Request for (%s) failed with error %d, and HTTP response code: %ld", url.c_str(), result, httpStatusCode);
					});
				}
				break;
