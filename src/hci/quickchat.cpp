...
namespace INTERNAL_ADMIN_ACTION_NOTICE {
	...
	std::string to_output_string(WzQuickChatMessageData messageData)
	{
		...
		bool selfChange = responsiblePlayerIdx == targetPlayerIdx;
		...
		switch (messageData.dataContext)
		{
			...
			case static_cast<uint32_t>(Context::Team):
				if (selfChange)
				{
					return astringf(_("Player (%s) changed team to: %d"), targetPlayerName, NetPlay.players[targetPlayerIdx].team);
				}
				else
				{
			...
			case static_cast<uint32_t>(Context::Position):
				if (selfChange)
				{
					return astringf(_("Player (%s) changed position to: %d"), targetPlayerName, NetPlay.players[targetPlayerIdx].position);
				}
				else
				{
			...
			case static_cast<uint32_t>(Context::Color):
				if (selfChange)
				{
					return astringf(_("Player (%s) changed color to: %s"), targetPlayerName, getPlayerColourName(targetPlayerIdx));
				}
				else
				{
			...
			case static_cast<uint32_t>(Context::Faction):
				if (selfChange)
				{
					return astringf(_("Player (%s) changed faction to: %s"), targetPlayerName, to_localized_string(static_cast<FactionID>(NetPlay.players[targetPlayerIdx].faction)));
				}
				else
				{
