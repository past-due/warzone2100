...
public:
	...
	bool prefValuesFromJSON(const nlohmann::json& j);
...
inline void from_json(const nlohmann::json& j, PlayerPreferences::BuiltinPreferences& builtinPreferences) {
	builtinPreferences.scavengers = setUint8ToSetEnumT<ScavType>(j.at("scavs").get<std::set<uint8_t>>(), validateNumToScavTypeEnum<uint8_t>);
	builtinPreferences.alliances = setUint8ToSetEnumT<AllianceType>(j.at("allies").get<std::set<uint8_t>>(), validateNumToAlliancesTypeEnum<uint8_t>);
	builtinPreferences.power = setUint8ToSetEnumT<PowerSetting>(j.at("power").get<std::set<uint8_t>>(), validateNumToPowerSettingEnum<uint8_t>);
	builtinPreferences.base = setUint8ToSetEnumT<CampType>(j.at("base").get<std::set<uint8_t>>(), validateNumToCampTypeEnum<uint8_t>);
	builtinPreferences.techLevel = setUint8ToSetEnumT<TechLevel>(j.at("tech").get<std::set<uint8_t>>(), validateNumToTechLevelEnum<uint8_t>);
	const auto& playersObj = j.at("players");
	builtinPreferences.minPlayers = clampMinMaxPlayersPrefValue(playersObj.at("min").get<uint8_t>());
	builtinPreferences.maxPlayers = clampMinMaxPlayersPrefValue(playersObj.at("max").get<uint8_t>());
}
...
nlohmann::json PlayerPreferences::prefValuesToJSON()
{
	...
	// TODO: Custom host-provided options?

	return result;
}

bool PlayerPreferences::prefValuesFromJSON(const nlohmann::json& j)
{
	try {
...
void loadMultiOptionPrefValues(const char *sPlayerName, uint32_t playerIndex)
{
	...
	playerPreferences[playerIndex].prefValuesFromJSON(result.value());
}

bool saveMultiOptionPrefValues(const char *sPlayerName, uint32_t playerIndex)
{
	auto multiOptionPrefJSON = playerPreferences[playerIndex].prefValuesToJSON();
	auto dumpedJSON = multiOptionPrefJSON.dump(4);
...
void recvPlayerMultiOptPreferencesBuiltin(int32_t sender, PlayerPreferences::BuiltinPreferences&& builtinPreferences)
{
	ASSERT_OR_RETURN(, sender < MAX_CONNECTED_PLAYERS, "Invalid sender: %" PRIu32, sender);
	playerPreferences[sender].getBuiltinPreferences() = std::move(builtinPreferences);
}
...
bool recvVote(NETQUEUE queue, bool inLobby)
{
	...
	switch (static_cast<NetVoteType>(voteType))
	{
		...
		case NetVoteType::LOBBY_OPTION_PREFERENCES_BUILTIN:
			...
			recvPlayerMultiOptPreferencesBuiltin(player, std::move(builtinPreferences));
			return true;
	}
