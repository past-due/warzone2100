...
/** A completed pathfinding result captured for GameState serialization (§7.1 Option C). A droid restored
 *  in MOVEWAITROUTE consumes this directly on its first resumed tick (fpathRoute), reproducing the exact
 *  host path without re-running the order-/context-sensitive pathfinder. */
...
/** Serialize side: if droid \p droidID has an in-flight path result, force it to completion and return it
 *  in \p out, re-populating the stored result so a host that keeps simulating can still consume it.
 *  Returns false if there is no pending result. NB blocks the caller until the worker finishes that job. */
bool fpathTakePendingResult(uint32_t droidID, FPathPendingResult &out);

/** Restore side: pre-populate droid \p droidID's path result so its first resumed MOVEWAITROUTE tick
