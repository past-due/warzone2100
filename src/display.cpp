...
void processGamepadCameraInput()
{
	...
	else if (rightX != 0.f || rightY != 0.f)
	{
		...
		const double panZoomFactor = 1.0 + ((getViewDistance() - MINDISTANCE) / static_cast<double>(MAXDISTANCE - MINDISTANCE));
		const double worldDeltaX = rightX * realTimeAdjustedIncrement(GAMEPAD_CAMERA_PAN_SPEED) * panZoomFactor;
		const double worldDeltaY = rightY * realTimeAdjustedIncrement(GAMEPAD_CAMERA_PAN_SPEED) * panZoomFactor;
