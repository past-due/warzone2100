...
private:
	...
	std::shared_ptr<WzWrappedGuideTopic> addTopicImpl(const std::shared_ptr<WzGuideTopic>& topic);
...
std::shared_ptr<WzWrappedGuideTopic> WzGuideTopicsRegistry::getCategoryTopic(const std::string& topicIdentifier)
{
	...
	while (lastComponentPos > 0)
	{
		...
		{
			debug(LOG_INFO, "Found parent: %s", possibleParent.c_str());
		...
		else
		{
			...
			else
			{
				...
				if (!categoryTopic)
				{
					debug(LOG_INFO, "No category: %s", possibleParent.c_str());
					nonCategoryTopicIds.insert(possibleParent);
					continue;
				}
				// add the category topic to the topics list, and return it
				debug(LOG_INFO, "Loaded category: %s", possibleParent.c_str());
...
std::shared_ptr<WzWrappedGuideTopic> WzGuideTopicsRegistry::addTopicImpl(const std::shared_ptr<WzGuideTopic>& topic)
{
	ASSERT_OR_RETURN(nullptr, topic != nullptr, "Null topic");

	if (identifierLookupMap.find(topic->identifier) != identifierLookupMap.end())
	{
		debug(LOG_INFO, "Guide topic with identifier \"%s\" was already added", topic->identifier.c_str());
		return nullptr;
	}

	if (topic->applicableModes.has_value() && !topic->applicableModes.value().empty())
	{
		if (topic->applicableModes.value().count(ActivityManager::instance().getCurrentGameMode()) == 0)
		{
			...
			debug(LOG_INFO, "Skipping topic (not applicable to current game mode): %s", topic->identifier.c_str());
			return nullptr;
		}
	}
...
std::vector<std::string> WzGuideTopicsRegistry::saveLoadedTopics() const
{
	...
	for (const auto& wrappedTopic : topics)
	{
		result.push_back(wrappedTopic->topic->identifier);
	}
	return result;
}
	...
	if (!foundLinkedTopicIdentifiers.empty())
	{
		...
		m_linkedTopicsWidget = WzGuideLinkedTopicsWidget::make(foundLinkedTopicIdentifiers, registry, [psWeakGuideTopicWidg](const std::string& linkedTopicIdentifier) {
			...
			widgScheduleTask([linkedTopicIdentifierCopy, psWeakGuideTopicWidg]() {
				...
				if (!strongGuideForm)
				{
					debug(LOG_INFO, "Clicked linked topic: %s", linkedTopicIdentifierCopy.c_str());
				}
...
std::vector<WzGuideVisual> loadWZGuideVisualsFromJSON(const nlohmann::json& j, const std::string& baseDir)
{
	...
	if (j.is_array())
	{
		...
		for (size_t i = 0; i < j.size(); ++i)
		{
			...
			// TODO: Handle imagePath with baseDir?
		}
	}
	else if (j.is_object())
	{
