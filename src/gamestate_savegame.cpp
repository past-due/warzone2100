...
// A real zip - not the previous framed-zlib blob - so the container is extractable/inspectable with
// any zip tool. Carrying every half in one document keeps the savegame self-contained while still
// allowing the setup header to be read BEFORE mods are mounted / the level loaded.
...
// A network joiner gets these re-fed by the host's GAME_* relay, so this section is disk-only. Raw
...
bool coldLoadRemountModsAndResolveLevel(const SetupHeaderInfo &header)
{
	...
	// 3. Warn-and-continue mod validation (never refuse: players legitimately update mods mid-play).
...
//
// A cold load restores the world at level-load time, but its scripting section must wait until
// stageThreeInitialise has prepared the scripts. We stash the GameState document between the two.
...
static bool g_coldLoadScriptingPending = false;
...
bool isColdLoadScriptingPending()
{
	return g_coldLoadScriptingPending;
}

void applyDeferredScripting()
{
	if (!g_coldLoadScriptingPending || !g_coldLoadGameStateDoc)
	{
		g_coldLoadScriptingPending = false;
		g_coldLoadGameStateDoc.reset();
		return;
	}
	try
	{
		// Scripts have been instantiated by prepareScripts() at this point, so the scripting
		// section's loadScriptStates() will actually take effect (scriptsAreReady() is now true).
...
bool coldLoadRestoreWorld()
{
	...
	try
	{
		...
		// objects, power/research/production/etc. The scripting section is skipped here (scripts are
		// not prepared until the stageThreeInitialise that runs right after this) and is replayed by
		// applyDeferredScripting() after prepareScripts().
	...
	g_coldLoadScriptingPending = true;
	return true;
}
...
LoadingTask<> coldLoadGameInit(ResourceLoadingController &controller, const std::string &folderPath)
{
	...
	// Load the level in RECONSTRUCT mode (Hybrid A): the normal scenario load runs - so the map's
