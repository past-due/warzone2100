...
{
	int maxTracks = static_cast<int>(floor(float(GetTrackListHeight() - TL_Y - W_TRACK_HEADER_Y) / float(TL_ENTRYH)));
	return maxTracks;
}
...
{
public:
	WzMusicModeCheckboxButton(WIDGET *parent, MusicGameMode mode, bool isChecked)
		: W_BUTTON(parent)
		, mode(mode)
		, isChecked(isChecked)
	{
		addOnClickHandler([](W_BUTTON& button) {
			WzMusicModeCheckboxButton& self = static_cast<WzMusicModeCheckboxButton&>(button);
...
void WzMusicModeCheckboxButton::display(int xOffset, int yOffset)
{
	int x0 = xOffset + x();
	int y0 = yOffset + y();
...
void WzMusicModeCheckboxButton::highlight(W_CONTEXT *psContext)
{
	if (!isEnabled()) return;
	W_BUTTON::highlight(psContext);
}

void WzMusicModeCheckboxButton::highlightLost()
{
	if (!isEnabled()) return;
...
{
public:
	W_TRACK_ROW(W_BUTINIT const *init, std::shared_ptr<const WZ_TRACK> track, bool ingame);
...
private:
	...
	std::vector<WzMusicModeCheckboxButton*> musicModeCheckboxes;
	MusicGameMode musicMode = MusicGameMode::MENUS;
};

W_TRACK_ROW::W_TRACK_ROW(W_BUTINIT const *init, std::shared_ptr<const WZ_TRACK> track, bool ingame)
	: W_BUTTON(init)
	, track(track)
{
	...
	{
		WzMusicModeCheckboxButton *pCheckBox = new WzMusicModeCheckboxButton(this, static_cast<MusicGameMode>(musicModeIdx), PlayList_TrackEnabledForMusicMode(track, static_cast<MusicGameMode>(musicModeIdx)));
...
void W_TRACK_ROW::geometryChanged()
{
...
void W_TRACK_ROW::display(int xOffset, int yOffset)
{
	...
	if ((musicMode != MusicGameMode::MENUS) && !PlayList_TrackEnabledForMusicMode(track, musicMode))
	{
...
{
public:
	WzMusicListHeaderColImage(WIDGET *parent)
	: W_BUTTON(parent)
	{
...
static void addTrackList(WIDGET *parent, bool ingame)
{
	WzMusicListHeader *pHeader = new WzMusicListHeader(parent);
	...
	for (int musicModeIdx = 0; musicModeIdx < NUM_MUSICGAMEMODES; musicModeIdx++)
	{
		...
		WzMusicListHeaderColImage* musicModeHeader = new WzMusicListHeaderColImage(pHeader);
	...
	{
		W_TRACK_ROW *pTrackRow = new W_TRACK_ROW(&emptyInit, trackList[i], ingame);
		pTrackRow->setGeometry(0, 0, TL_ENTRYW, TL_ENTRYH);
		pTrackRow->addOnClickHandler([](W_BUTTON& clickedButton) {
			W_TRACK_ROW& pTrackRow = static_cast<W_TRACK_ROW&>(clickedButton);
			if (selectedTrack == pTrackRow.getTrack()) { return; }
			cdAudio_PlayTrack(pTrackRow.getTrack(), [](const std::shared_ptr<const WZ_TRACK> &track) { });
		});
