...
#define q(i,j,center) ((x*ySectors+y)*(sectorSize+1)*(sectorSize+1)*2 + ((i)*(sectorSize+1)+(j))*2+(center))
	...
	auto lightmapChannels = gfx_api::context::get().getClosestSupportedUncompressedImageFormatChannels(3, iV_Image::ColorSpace::Linear); // ideal is RGB, but check if supported
	ASSERT_OR_RETURN(false, lightmapChannels.has_value(), "Exhausted all possible uncompressed formats for lightmap texture??");

	if (lightmapPixmap == nullptr || !lightmapPixmap->allocate(lightmapWidth, lightmapHeight, lightmapChannels.value(), true, iV_Image::ColorSpace::Linear))
	{
