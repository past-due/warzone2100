...
template<typename PSO>
static void drawTerrainCombinedTessImpl(const glm::mat4 &ModelViewProjection, const glm::mat4& ViewMatrix, const glm::mat4 &ModelUVLightmap, const Vector3f &cameraPos, const Vector3f &sunPos, const ShadowCascadesInfo& shadowCascades)
{
	if (!terrainDecalVBO || !terrainPatchIndexVBO)
	{
		return; // no terrain+decal geometry (empty map)
	...
	if (terrainMeshStrategy == TerrainMeshStrategy::HardwareTess)
	{
		switch (terrainShaderQuality)
		{
			case TerrainShaderQuality::MEDIUM:
				drawTerrainCombinedTessImpl<gfx_api::TerrainCombinedTess_Medium>(ModelViewProjection, ViewMatrix, ModelUVLightmap, cameraPos, sunPos, shadowCascades);
				return;
			case TerrainShaderQuality::NORMAL_MAPPING:
				drawTerrainCombinedTessImpl<gfx_api::TerrainCombinedTess_High>(ModelViewProjection, ViewMatrix, ModelUVLightmap, cameraPos, sunPos, shadowCascades);
				return;
			default:
...
void drawWaterNormalImpl(const glm::mat4 &ModelViewProjection, const Vector3f &cameraPos, const Vector3f &sunPos)
{
	...
	PSO::get().bind_constants({
		...
		waterOffset*10, gfx_api::context::get().getMipLodBias()
	});
