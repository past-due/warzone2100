...
/// True between coldLoadGameInit() and applyDeferredScripting() - i.e. a new-format cold load is
/// mid-flight and its scripting section still needs replaying once scripts are prepared.
bool isColdLoadScriptingPending();

/// Replay the cold-load's deferred scripting section. Call once, AFTER prepareScripts() in
/// stageThreeInitialise. No-op unless a cold load is pending.
