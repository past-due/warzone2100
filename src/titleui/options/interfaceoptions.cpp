...
std::shared_ptr<OptionsForm> makeInterfaceOptionsForm(bool inGame, const std::function<void()> languageDidChangeHandler)
{
	...
	}
	{
		...
		auto valueChanger = OptionsDropdown<uint8_t>::make(
			[]() {
				...
				{
					// add "Custom" item
					result.choices.push_back({ WzString::format("(Custom: %u)", static_cast<unsigned>(currValue)), "", currValue });
					result.currentIdx = result.choices.size() - 1;
				}
				return result;
			},
	...
	{
		auto optionInfo = OptionInfo("interface.multiplayer.chat", N_("Chat"), N_("Whether to enable free chat in online multiplayer games. Does not affect local skirmish / challenge / campaign games."));
