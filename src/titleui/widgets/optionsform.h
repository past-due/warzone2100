...
// UpdatableOptionsInterface
// - OptionsForm::addOption takes an optionValueChangerWidget, which is expected to be a custom WIDGET subclass that also implements UpdatableOptionsInterface to handle certain events
class UpdatableOptionsInterface
{
public:
	virtual ~UpdatableOptionsInterface();
...
// OptionsDropdown<T>
// - A custom DropdownWidget subclass with a simplified interface that handles UpdatableOptionsInterface
//
// Example Usage:
// ```cpp
...
//		},
//		// onSelect
//		[](size_t idx, const auto& choices) -> bool {
//			auto newValue = choices[idx].value;
//			// TO IMPLEMENT: Set / store / persist the new option value
//		},
...
template<typename T>
class OptionsDropdown : public WzOptionsDropdownWidget, public UpdatableOptionsInterface
{
protected:
	OptionsDropdown()
	{ }
public:
	...
	typedef std::function<bool (size_t idx, const std::vector<OptionChoice<T>>& choices)> OnSelectItemFunc; // note: return true if able to select item, false if a failure
public:
	static std::shared_ptr<OptionsDropdown<T>> make(PopulateFunc populate, OnSelectItemFunc onSelect, bool needsStateUpdates, int maxListItemsShown = 5)
	{
		...
		auto psWeakParent = std::weak_ptr<OptionsDropdown<T>>(std::dynamic_pointer_cast<OptionsDropdown<T>>(result->shared_from_this()));
		result->setCanChange([psWeakParent](DropdownWidget &widget, size_t newIndex, std::shared_ptr<WIDGET> newSelectedWidget) -> bool {
			auto strongParent = psWeakParent.lock();
			ASSERT_OR_RETURN(false, strongParent != nullptr, "No parent?");
			ASSERT_OR_RETURN(false, newIndex < strongParent->current.choices.size(), "Invalid index");
			if (strongParent->current.choices[newIndex].disabled) { return false; }
			return strongParent->onSelectFunc(newIndex, strongParent->current.choices);
		});
...
//		setOnChange([handler](DropdownWidget& widg) {
//			handler(widg);
//		});
		setOnChange(handler);
	}
...
public:
	...
	template<typename T>
	std::enable_if_t<std::is_base_of<UpdatableOptionsInterface, T>::value && std::is_base_of<WIDGET, T>::value> addOption(const OptionInfo& optionInfo, const std::shared_ptr<T>& optionValueChangerWidget, bool bottomBorder = false, int16_t indentLevel = 0)
	{
		addOptionInternal(optionInfo, optionValueChangerWidget, std::static_pointer_cast<UpdatableOptionsInterface>(optionValueChangerWidget), bottomBorder, indentLevel);
	}

	// Add a section heading to the options form
	void addSection(const OptionsSection& optionsSection);
...
private:
	void addOptionInternal(const OptionInfo& optionInfo, const std::shared_ptr<WIDGET>& optionValueChangerWidget, const std::shared_ptr<UpdatableOptionsInterface>& pUpdatableOptionsInterface,  bool bottomBorder = false, int16_t indentLevel = 0);
...
private:
	...
	std::unordered_map<WzString, size_t> sectionIdxMap; // maps sectionId -> index in scrollable list widget
