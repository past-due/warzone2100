...
// MARK: - UpdatableOptionsInterface

UpdatableOptionsInterface::~UpdatableOptionsInterface()
{ }
...
void OptionRow::display(int xOffset, int yOffset)
{
	...
	if (showBottomBorder)
	{
		...
		int bottomBorderY0 = y1 /*- outerPaddingY*/ - 1;
...
void OptionsForm::addSection(const OptionsSection& optionsSection)
{
	// TODO: Make widget that displays the section title
}

void OptionsForm::addOptionInternal(const OptionInfo& optionInfo, const std::shared_ptr<WIDGET>& optionValueChangerWidget, const std::shared_ptr<UpdatableOptionsInterface>& pUpdatableOptionsInterface2, bool bottomBorder /*= false*/, int16_t indentLevel /*= 0*/)
{
	...
	auto pUpdatableOptionsInterface = std::dynamic_pointer_cast<UpdatableOptionsInterface>(optionValueChangerWidget);
	ASSERT_OR_RETURN(, pUpdatableOptionsInterface != nullptr, "optionValueChangerWidget does not implement appropriate interface");
	WzString optionIdCopy = optionInfo.optionId;
	pUpdatableOptionsInterface->addOnChangeHandler([pWeakOptionsForm, optionIdCopy](WIDGET&) {
	...
	pUpdatableOptionsInterface->addCloseFormManagedPopoversHandler([pWeakOptionsForm](WIDGET&) {
		auto strongOptionsForm = pWeakOptionsForm.lock();
		ASSERT_OR_RETURN(, strongOptionsForm != nullptr, "Invalid pointer");
	...
	pUpdatableOptionsInterface->informAvailable(isAvailable);
	...
	if (rowWidget != priorMouseOverRowWidget)
	{
		...
		if (psOptionRowWidget)
		{
			...
			if (it != optionDetailsMap.end())
			{
				...
				auto pUpdatableOptionsInterface = std::dynamic_pointer_cast<UpdatableOptionsInterface>(wrappedOptionValueChangerWidget->getOptionValueChangerWidget());
				std::vector<OptionChoiceHelpDescription> choiceHelpDescriptions;
				if (pUpdatableOptionsInterface)
				{
					auto optHelpDescriptions = pUpdatableOptionsInterface->getHelpDescriptions();
...
void OptionsForm::refreshOptionsInternal(const std::unordered_set<WzString>& except)
{
	...
	for (auto& i : optionDetailsMap)
	{
		...
		if (pUpdatableOptionsInterface)
		{
			pUpdatableOptionsInterface->update(forceUpdate);
			pUpdatableOptionsInterface->informAvailable(isAvailable);
		}
