...
std::shared_ptr<OptionsForm> HostingAdvancedOptionsForm::makeHostOptionsForm()
{
	...
	{
		auto optionInfo = OptionInfo("mp.hostPosition", N_("Host Role"), N_(""));
		auto valueChanger = OptionsDropdown<bool>::make(
			...
			[psWeakParent](size_t idx, const auto& choices) -> bool {
				auto strongParent = psWeakParent.lock();
				ASSERT_OR_RETURN(false, strongParent != nullptr, "No parent?");
				return strongParent->setSpectatorHost(choices[idx].value);
			}, false
		);
	...
	}
	{
		...
		auto valueChanger = OptionsDropdown<BLIND_MODE>::make(
			...
			[psWeakParent](size_t idx, const auto& choices) -> bool {
				auto strongParent = psWeakParent.lock();
				ASSERT_OR_RETURN(false, strongParent != nullptr, "No parent?");
				auto newBlindModeValue = choices[idx].value;
				return strongParent->setBlindMode(newBlindModeValue);
			}, true
		);
		result->addOption(optionInfo, valueChanger);
	}
...
void WzStartHostingButton::display(int xOffset, int yOffset)
{
	...
	auto background_color = isDown || isDisabled ? pal_RGBA(10, 0, 70, 250) : pal_RGBA(25, 0, 110, 220);
	if (isDisabled)
	{
	...
	int backX0 = x0;
	int backY0 = y0;
	pie_BoxFill(backX0, backY0, backX0 + w, backY0 + h, (isHighlight) ? WZCOL_TEXT_BRIGHT : WZCOL_TEXT_MEDIUM);
	pie_BoxFill(backX0 + 1, backY0 + 1, backX0 + w - 1, backY0 + h - 1, WZCOL_MENU_BACKGROUND);
