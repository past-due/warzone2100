...
void WzLobbyMultiOptionButton::display(int xOffset, int yOffset)
{
	...
	if (localVoteIndicator || (isHighlight && showLocalVoteIndicatorOnHighlight))
	{
		...
		iV_DrawImageFileAnisotropicTint(FrontImages, IMAGE_THUMBS_UP, imgPosX0, imgPosY0, Vector2f(voteIndicatorDimensions, voteIndicatorDimensions), voteColor);
	}
}
...
protected:
	...
	void initialize(bool isChallenge);
public:
	static std::shared_ptr<WzMultiLobbyOptionsImpl> make(bool isChallenge);
...
std::shared_ptr<WzMultiLobbyOptionsImpl> WzMultiLobbyOptionsImpl::make(bool isChallenge)
{
...
void WzMultiLobbyOptionsImpl::initialize(bool _isChallenge)
{
	...
	auto wrapItemForList = [&](const std::shared_ptr<WIDGET>& widg, WIDGET_DISPLAY displayFunction = nullptr) {
		...
		wrappedWidget->displayFunction = displayFunction;
	...
	mapChangeButton->addOnClickHandler([](W_BUTTON&) {
		auto multiOptionsTitleUI = std::dynamic_pointer_cast<WzMultiplayerOptionsTitleUI>(wzTitleUICurrent);
		ASSERT_OR_RETURN(, multiOptionsTitleUI != nullptr, "Unexpected title UI");
		multiOptionsTitleUI->openMapChooser(); // FUTURE TODO: Replace this with a new map chooser overlay screen
	});
	if (isChallenge || ingame.side == InGameSide::MULTIPLAYER_CLIENT)
	{
	...
	limitsInfoBut->addOnClickHandler([](W_BUTTON&) {
		debug(LOG_INFO, "On click");
		// FIXME: TODO: Print out limits info to console
	});
	if (ingame.side == InGameSide::HOST_OR_SINGLEPLAYER)
	{
...
std::shared_ptr<PopoverMenuWidget> WzMultiLobbyOptionsImpl::buildMatchOptionsMenu()
{
	...
	if ((ingame.side == InGameSide::MULTIPLAYER_CLIENT) && !(NetPlay.isHost))
	{
		...
		addClickableMenuItem(popoverMenu, _("Indicate Your Preferences:"), nullptr, true);
		// Enable All Options
		addClickableMenuItem(popoverMenu, _("Like All"), [](WIDGET_KEY) {
			if ((ingame.side == InGameSide::MULTIPLAYER_CLIENT) && !(NetPlay.isHost))
			{
	...
	if ((ingame.side == InGameSide::HOST_OR_SINGLEPLAYER) && !isChallenge)
	{
		addClickableMenuItem(popoverMenu, _("Player Preferences:"), nullptr, true);
...
{
	return WzMultiLobbyOptionsImpl::make(isChallenge);
}
