...
static bool startTitleMenu()
{
	...
	pRightAlignedButton->move(pRightAlignedButton->parent()->width() - pRightAlignedButton->width(), pRightAlignedButton->y());
...
W_BUTTON * addSmallTextButton(UDWORD id,  UDWORD PosX, UDWORD PosY, const char *txt, unsigned int style)
{
	...
	if (!(style & WBUT_TXTCENTRE))
	{
		sButInit.width = (uint16_t)(iV_GetTextWidth(txt, font_small)) + 10;
		sButInit.x += 35;
	}
	else
	{
