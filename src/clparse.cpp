...
static const struct poptOption *getOptionsTable()
{
	static const struct poptOption optionsTable[] =
	{
		...
		{ "gamestate-script-dump-tick", POPT_ARG_STRING, CLI_GAMESTATE_SCRIPTDUMP, N_("At this game tick, dump the full script state (all instances) to <crc-trace-file>.scriptstate.<tick>.json (diff original vs loaded run to pinpoint a divergent script global/label/timer)"), N_("game tick") },
...
bool ParseCommandLine(int argc, const char * const *argv)
{
	...
	while ((iOption = poptGetNextOpt(poptCon)) > 0)
	{
		...
		case CLI_GAMESTATE_CRCDETAIL_ONSAVE:
			...
			break;
		case CLI_GAMESTATE_SCRIPTDUMP:
			token = poptGetOptArg(poptCon);
			if (token == nullptr)
			{
				qFatal("Missing game tick value for --gamestate-script-dump-tick");
			}
			gamestate::gamestateSetScriptStateDumpTick(static_cast<uint32_t>(atoi(token)));
			break;
