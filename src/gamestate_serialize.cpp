                           ...
                           std::unordered_map<uint32_t, DROID *> &droidById)
{
	...
	const Position pos(jpos[0].get<int32_t>(), jpos[1].get<int32_t>(), jpos[2].get<int32_t>());
		...
		for (const MESSAGE *psMessage : apsMessages[player])
		{
			...
			if (psMessage->type == MSG_PROXIMITY)
			{
				...
				if ((*it)->type == POS_PROXDATA)
				{
					...
					{
						const VIEW_PROXIMITY *vp = vd != nullptr ? reinterpret_cast<VIEW_PROXIMITY *>(vd->pData) : nullptr;
						if (vp != nullptr)
						{
							m["beaconX"] = vp->x;
							m["beaconY"] = vp->y;
			...
			else
			{
				...
				if (psMessage != nullptr)
				{
					...
					if (psViewData != nullptr)
					{
						...
						VIEW_PROXIMITY *vp = reinterpret_cast<VIEW_PROXIMITY *>(psViewData->pData);
