...
static BatchedSelectionRenderer batchedSelectionRenderer;
...
static void drawTiles(iView *player)
{
	...
	// draw it
	drawTerrain(perspectiveViewMatrix * glm::translate(glm::vec3(-player->p.x, 0, player->p.z)));
...
bool init3DView()
{
	...
	batchedSelectionRenderer.initialize();
...
void shutdown3DView()
{
	...
	batchedSelectionRenderer.reset();
}
...
void renderDeliveryPoint(FLAG_POSITION *psPosition, bool blueprint, const glm::mat4& viewMatrix, const glm::mat4 &perspectiveViewMatrix)
{
	...
	iIMDShape *psIMD = pAssemblyPointIMDs[psPosition->factoryType][psPosition->factoryInc];
	if (psIMD != nullptr)
	{
		pie_Draw3DShape(psIMD, 0, 0, colour, pieFlag, pieFlagData, viewMatrix * modelMatrix);
	}
...
static void	drawStructureSelections()
{
	...
	if (isSpectator)
	{
		if (barMode == BAR_DROIDS_AND_STRUCTURES)
		{
			...
			{
				batchedSelectionRenderer.addStructureSelectionsToRender(player, psClickedOn, bMouseOverOwnStructure);
			}
		}
		return;
	}
	...
	batchedSelectionRenderer.addStructureSelectionsToRender(selectedPlayer, psClickedOn, bMouseOverOwnStructure);

	batchedSelectionRenderer.addStructureTargetingGfxToRender();
	...
	if (bMouseOverStructure && !bMouseOverOwnStructure)
	{
		if (mouseDown(getRightClickOrders() ? MOUSE_LMB : MOUSE_RMB))
		{
			...
			batchedSelectionRenderer.addEnemyStructureHealthToRender(psStruct);
		}
	}
...
static void	drawDroidSelections()
{
	...
	if (isSpectator)
	{
		if (barMode == BAR_DROIDS || barMode == BAR_DROIDS_AND_STRUCTURES)
		{
			for (int player = 0; player < MAX_PLAYERS; player++)
			{
				batchedSelectionRenderer.addDroidSelectionsToRender(player, psClickedOn, bMouseOverDroid);
			}
		}
	...
	batchedSelectionRenderer.addDroidSelectionsToRender(selectedPlayer, psClickedOn, bMouseOverDroid);
	...
	if (bMouseOverDroid && !bMouseOverOwnDroid)
	{
		if (mouseDown(getRightClickOrders() ? MOUSE_LMB : MOUSE_RMB))
		{
			if (psClickedOn->player != selectedPlayer && psClickedOn->sDisplay.frameNumber == currentGameFrame)
			{
				...
				batchedSelectionRenderer.addEnemyDroidHealthToRender(psDroid);
			}
		}
	}
...
static void drawDroidAndStructureSelections()
{
	...
	batchedSelectionRenderer.drawAllSelections();
	batchedSelectionRenderer.clear();
}
