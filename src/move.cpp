...
bool moveSetFormationSpeedLimiting(uint32_t player, bool enabled)
{
	...
	if (myResponsibility(player))
	{
		...
		NETbeginEncode(NETgameQueue(currentPlayer), GAME_FORMATION_OPT);
		NETuint8_t(&currentPlayer);		// player
		NETbool(&enabled);				// formation speed limiting value
		return NETend();
	}
	else
	{
...
bool recvSetFormationOpt(NETQUEUE queue)
{
	uint8_t player;
	bool	speedLimitingEnabled;

	NETbeginDecode(queue, GAME_FORMATION_OPT);
	NETuint8_t(&player);            			// the player
	NETbool(&speedLimitingEnabled);
	NETend();

	if (!canGiveOrdersFor(queue.index, player))
	{
		debug(LOG_WARNING, "Formation speed limiting (by %d) for wrong player (%d).", queue.index, player);
	...
	playerFormationSpeedLimiting[player] = speedLimitingEnabled;
