...
/// Lockstep network-timing state captured for a GameState snapshot, so a resumed/joining client
/// continues with the same latency negotiation and per-queue command scheduling instead of starting
/// from defaults (which would shift the `lat` in GAME_GAME_TIME and the tick at which queued commands
