...
namespace gfx_api
{
	...
	struct pipeline_state_helper
	{
		...
		void recompile()
		{
			this->nextpso = gfx_api::context::get().build_pipeline(rasterizer::get(), shader, primitive, untuple<texture_input>(texture_inputs{}), untuple<vertex_buffer>(vertex_buffer_inputs{}));
		}
	private:
