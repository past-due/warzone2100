...
/// bytes. For disk savegames: a host relay re-feeds in-flight commands to a network joiner, but a disk
/// cold-load has no relay, so the backlog must be persisted and re-injected. Empty for an unallocated slot.
