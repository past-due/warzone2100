...
void widgShutDown(void)
{
	...
	{
		// There appear to be reference cycles, or widgets that weren't cleaned up properly (or at the right time)
...
static WIDGET *widgFormGetFromID(WIDGET *widget, UDWORD id)
{
	...
	{
		WIDGET *w = widgFormGetFromID(child.get(), id);
		if (w != nullptr)
		{
			return w;
		}
...
{
	ASSERT_OR_RETURN(nullptr, psScreen != nullptr, "Invalid screen pointer");
	return widgFormGetFromID(psScreen->psForm.get(), id);
}
...
bool WIDGET::processClickRecursive(W_CONTEXT *psContext, WIDGET_KEY key, bool wasPressed)
{
	...
	if (auto lockedScreen = screenPointer.lock())
	{
		if (isMouseOverWidget() && !lockedScreen->isLastHighlight(*this))
		{
