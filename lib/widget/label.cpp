...
void W_LABEL::display(int xOffset, int yOffset)
{
	...
	{
		textBoundingBoxOffset.y = yOffset + y() /*- displayCache.wzText[0].aboveBase()*/;
	}
	else if ((style & WLAB_ALIGNBOTTOMLEFT) != 0)  // Align bottom
	{
		textBoundingBoxOffset.y = yOffset + y() /*- displayCache.wzText[0].aboveBase()*/ + (height() - textTotalHeight);
//		textBoundingBoxOffset.y = yOffset + y() - displayCache.wzText[0].aboveBase() + (height() - displayCache.wzText[0].lineSize());
	}
	else
	{
		textBoundingBoxOffset.y = yOffset + y() /*- displayCache.wzText[0].aboveBase()*/ + (height() - textTotalHeight) / 2;
//		textBoundingBoxOffset.y = yOffset + y() - displayCache.wzText[0].aboveBase() + (height() - displayCache.wzText[0].lineSize()) / 2;
	}
...
//		baseLineOffset = -wzTextLine.aboveBase() - wzTextLine.height(); //-wzTextLine.aboveBase() + (wzTextLine.height() - wzTextLine.lineSize());
//		std::min<int>(wzTextLine.aboveBase() + wzTextLine.height(), 0);

//		float fy = float(textBoundingBoxOffset.y) + float(jy) + float(wzTextLine.height() - wzTextLine.lineSize()) / 2.f - float(wzTextLine.aboveBase());
		float fy = float(textBoundingBoxOffset.y) + float(jy) - float(wzTextLine.aboveBase());//float(wzTextLine.height() - wzTextLine.lineSize()) / 2.f - float(wzTextLine.aboveBase());

//		if (displayCache.wzText.size() > 1)
//		{
//			debug(LOG_3D, "Displaying line (height: %d; linesize: %d; aboveBase: %d; belowBase: %d; baseLineOffset: %d; fy: %f): %s", wzTextLine.height(), wzTextLine.lineSize(), wzTextLine.aboveBase(), wzTextLine.belowBase(), baseLineOffset, fy, wzTextLine.getText().c_str());
//		}
...
#endif
		wzTextLine.render(textBoundingBoxOffset.x + fx, /*textBoundingBoxOffset.y + jy + baseLineOffset*/fy, fontColour);
