...
if [[ "$TARGET_BUILD_ARCH" == "amd64" && "$CRAFT_ARCH_BUILD_ON" == "$TARGET_BUILD_ARCH" ]]; then
  ...
  VULKANSDK_DL_TMPPATH="${CRAFT_PART_BUILD}/dl_tmp"
  
  wget -qO "${VULKANSDK_DL_TMPPATH}" "${VULKANSDK_DLURL}"
  VULKAN_DL_FILE="${VULKANSDK_DL_TMPPATH}/vulkansdk-linux-x86_64-1.4.321.1.tar.xz"
  
  echo "${VULKANSDK_SHA256} ${VULKAN_DL_FILE}" | sha256sum -c
  mkdir "${VULKANSDK_INSTALL_PATH}"
